...
# Active Learning and Few Shot Supervised Learning
Generating labels is expensive. Usually it involves human annotators; often it is messy and ambiguous. 
...
**Low / Few  Shot / Semi- Supervised** learning tries to do a lot with [few initial labels](https://arxiv.org/pdf/2110.02922.pdf).

* say you have a search approach that suggests lines of code that might have bugs. We have many goals to optimize for: different parameters unique to our application, security bugs vs other bugs, who authored the code, when it was committed, etc. 
* black-box: unclear how solutions are generated, so ask humans what they prefer
* but how much can we ask human raters? 

----

# Semi-supervised
Train data on a few amount of labels with lots of unlabeled data assuming:
- the data is largely the same wrt dimensions 
- data is continuous along dimensions (close in space = close semantically)
- clusters represent multiple items
- often able to get close to similar performance with fractions of labels required

----

## Genetic Algorithms

We will cover these in a later week.

- NSGAII
- Diff Evolution 

[JMetal](https://jmetal.github.io/jMetal/) is a great GA library/framework.
