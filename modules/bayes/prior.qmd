...
We will **set** priors using statistical probability distributions (in R type `?Distributions`). These distributions cover a range of widely understood generative processes
...
One thing to note: libraries like Python's [Seaborn](https://seaborn.pydata.org/tutorial/distributions.html) (via the `kdeplot()` function for example) allow you to *fit distributions to the data* (in this case, using kernel density estimation to 'smooth' the underlying probability density). 
