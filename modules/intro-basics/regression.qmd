...
fit_1 <- lm(defects ~ loc, data=jm1, family=binomial)
```
...
For the frequentist linear regression approach, we have a metric such as $R^2$, which measures the goodness of fit or proportion of variance explained. A perfect linear model would have $R^2$ of 1.0, indicating *all* the variance in the data is captured in the model. For our `fit_1` object above, which is a logistic regression model, a better measure is the classification accuracy (defective/not defective). We can use the model's fitted values (`fit_1$fitted.values`) to compare. I'm not an expert in this, however. 
